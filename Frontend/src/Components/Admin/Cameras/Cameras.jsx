...
// Dark theme tokens -------------------------------------------------------
const statusRing = (s) => ({ online:'ring-green-500/80', degraded:'ring-orange-400/80', offline:'ring-red-600/80', unknown:'ring-gray-500/80' }[s] || 'ring-gray-500/80');
const statusBadge = (s) => ({
  ...
  offline:'bg-red-600/15 text-red-300 border-red-400/30',
  unknown:'bg-gray-500/15 text-gray-300 border-gray-400/30'
}[s] || 'bg-gray-500/15 text-gray-300 border-gray-400/30');
...
const Cameras = () => {
  ...
  const renderEmpty = () => (
    <div className="p-10 text-sm text-white/60 text-center border border-dashed border-white/10 rounded-lg bg-white/5 backdrop-blur flex flex-col items-center gap-3">
      <AlertTriangle className="text-orange-400" size={40} />
      No cameras configured yet.
    </div>
  );
  const renderError = () => (
    <div className="p-4 bg-red-600/15 text-red-300 text-sm flex items-center justify-between rounded border border-red-400/30">
      ...
      <AnimatePresence initial={false}>
        {filtered.map(cam => (
          <motion.button
            ...
            className="group relative rounded-lg border border-white/10 bg-white/5 backdrop-blur-md shadow-sm overflow-hidden focus:outline-none focus:ring-2 focus:ring-orange-500/50 flex flex-col hover:bg-white/10 transition"
            aria-label={`Open ${cam.name} stream`}
          >
            <div className={`aspect-video w-full bg-gray-800 ring-4 ${statusRing(cam.status)} ring-offset-0 flex items-center justify-center text-white/50 text-[11px] relative overflow-hidden`}>            
              <img src={cam.imageUrl+`&tick=${tick}`} loading="lazy" alt={cam.name+ ' thumbnail'} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105 opacity-80 group-hover:opacity-100" />
              <div className="absolute inset-0 bg-black/30 group-hover:bg-black/40 transition-colors" />
              <div className="absolute bottom-1 left-1 px-1.5 py-0.5 rounded bg-black/60 text-[10px] text-white flex items-center gap-1 backdrop-blur-sm"><Clock size={10}/> {new Date(cam.updatedAt).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</div>
            </div>
            <div className="p-2 flex flex-col gap-1 w-full">
              <div className="text-xs font-medium text-white/90 truncate flex items-center gap-1">
                {cam.status==='offline'? <CamOffIcon size={14} className="text-red-400"/> : <Camera size={14} className="text-orange-400"/>}
                {cam.name}
              </div>
              <div className="text-[11px] text-white/60 flex items-center gap-1">
                <span className="px-1 py-0.5 rounded bg-white/5 border border-white/10">{cam.zoneName}</span>
                <span className="ml-auto tabular-nums font-medium text-white/80">{cam.faceRatePerMin}<span className="text-[10px] text-white/40 ml-0.5">/min</span></span>
              </div>
            </div>
  ...
  // List view --------------------------------------------------------------
  const list = (
    <div className="overflow-x-auto border border-white/10 rounded-lg bg-white/5 backdrop-blur-md shadow-sm">
      <table className="min-w-full text-xs text-white/80">
        ...
        <tbody>
          {filtered.map(cam => (
            <motion.tr key={cam.id} layout initial={{opacity:0, y:6}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-4}} className="even:bg-white/5 hover:bg-white/10 focus-within:bg-white/10">
              <td className="px-3 py-2 whitespace-nowrap">
                <button onClick={()=>setModalCamera(cam)} className="flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-orange-500/50 rounded">
                  <div className={`w-14 h-8 rounded overflow-hidden ring-2 ${statusRing(cam.status)} bg-gray-800 flex items-center justify-center`}>
                    <img src={cam.imageUrl+`&tick=${tick}`} loading="lazy" alt={cam.name+' thumbnail'} className="w-full h-full object-cover opacity-80" />
                  </div>
                  <span className="font-medium text-white/90 flex items-center gap-1">{cam.status==='offline'? <CamOffIcon size={14} className="text-red-400"/> : <Camera size={14} className="text-orange-400"/>}{cam.name}</span>
                </button>
              </td>
              <td className="px-3 py-2 whitespace-nowrap text-white/70">{cam.zoneName}</td>
              ...
              <td className="px-3 py-2 whitespace-nowrap tabular-nums text-white/80">{cam.faceRatePerMin}</td>
              <td className="px-3 py-2 whitespace-nowrap text-white/60">{new Date(cam.updatedAt).toLocaleTimeString()}</td>
              <td className="px-3 py-2 whitespace-nowrap space-x-2">
                <button onClick={()=>setModalCamera(cam)} className="text-orange-300 hover:underline">View</button>
                <button className="text-white/60 hover:text-white/80 hover:underline">Edit</button>
                <button className="text-red-400 hover:text-red-300 hover:underline">Remove</button>
              </td>
            </motion.tr>
          ))}
        </tbody>
