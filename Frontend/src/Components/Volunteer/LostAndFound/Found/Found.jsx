...
const Found = ({ data, loading, onUpdate, onResolve }) => {
  ...
  return (
    <div className="flex flex-col md:grid md:grid-cols-5 md:gap-5" aria-label="Found cases list">
      <div className="md:col-span-2 space-y-3 mb-4 md:mb-0">
        ...
        {!loading && sorted.length===0 && <div className="p-10 text-center text-sm text-gray-500 bg-white border rounded-lg">No found cases.</div>}
        {sorted.map(c => (
      ...
      <div className="hidden md:block md:col-span-3">
        {detail ? (
          <div className="bg-white rounded-lg border border-gray-200 shadow-sm h-full p-5 flex flex-col" aria-label="Found case detail">
            <div className="flex items-start justify-between gap-3 mb-4">
              <div className="space-y-1 flex-1 min-w-0">
                <h2 className="text-sm font-semibold text-gray-800 leading-snug">{detail.type==='person'? 'Person':'Item'} Found</h2>
            ...
            <div className="space-y-3">
              {detail.status==='matched' && (
                <div className="flex gap-2">
                  <button onClick={()=>confirm(detail)} className="flex-1 h-10 rounded-md bg-green-600 text-white text-sm font-medium hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 flex items-center justify-center gap-2"><CheckCircle2 size={16}/> Confirm</button>
                  <button onClick={()=>reject(detail)} className="flex-1 h-10 rounded-md bg-red-600 text-white text-sm font-medium hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 flex items-center justify-center gap-2"><XCircle size={16}/> Reject</button>
                </div>
              )}
              {detail.status!=='resolved' && (
                <button onClick={()=>resolve(detail)} className="h-10 rounded-md w-full bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 flex items-center justify-center gap-2"><Flag size={16}/> Mark Resolved</button>
              )}
              {detail.status==='resolved' && <div className="text-[11px] text-green-600 font-medium">Case resolved.</div>}
            </div>
          </div>
        ) : (
          <div className="h-full border border-dashed rounded-lg flex items-center justify-center text-xs text-gray-500">Select a found case</div>
        )}
      </div>
        ...
        {detail && (
          <div className="space-y-4 text-sm">
            ...
            <div className="space-y-3 pt-2">
              {detail.status==='matched' && (
                <div className="flex gap-2">
                  <button onClick={()=>confirm(detail)} className="flex-1 h-10 rounded-md bg-green-600 text-white text-sm font-medium flex items-center justify-center gap-2"><CheckCircle2 size={16}/> Confirm</button>
                  <button onClick={()=>reject(detail)} className="flex-1 h-10 rounded-md bg-red-600 text-white text-sm font-medium flex items-center justify-center gap-2"><XCircle size={16}/> Reject</button>
                </div>
              )}
              {detail.status!=='resolved' && (
                <button onClick={()=>resolve(detail)} className="h-10 rounded-md w-full bg-blue-600 text-white text-sm font-medium flex items-center justify-center gap-2"><Flag size={16}/> Mark Resolved</button>
              )}
              {detail.status==='resolved' && <div className="text-[11px] text-green-600 font-medium">Case resolved.</div>}
            </div>
          </div>
