...
const Missings = ({ data, loading, onMarkFound }) => {
  ...
  return (
    <div className="space-y-3" aria-label="Missing cases">
      {loading && Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="h-24 rounded-lg bg-gradient-to-r from-gray-100 via-gray-200 to-gray-100 animate-pulse" />
      ))}
      {!loading && sorted.length === 0 && (
        <div className="p-10 text-center text-sm text-gray-500 bg-white border rounded-lg">No missing cases.</div>
      )}
      {sorted.map(c => (
        <div
          key={c.id}
            ...
            className="bg-white border border-gray-200 hover:border-orange-400 rounded-lg p-3 flex gap-3 items-center shadow-sm cursor-pointer"
        >
          <div className="h-14 w-14 rounded-md bg-gray-100 flex items-center justify-center overflow-hidden text-gray-400 text-[10px] font-medium">
            {c.photoUrls.length ? (
              <img src={c.photoUrls[0]} alt={c.type + ' thumbnail'} className="h-full w-full object-cover" />
            ) : c.type === 'person' ? 'PERSON' : 'ITEM'}
          </div>
      ...
      <Drawer open={!!detail} onClose={() => setDetail(null)} title={detail ? 'Missing ' + (detail.type === 'person' ? 'Person' : 'Item') : ''}>
        {detail && (
          <div className="space-y-4 text-sm">
            ...
            <div className="space-y-3 pt-2">
              {detail.status === 'missing' && (
                <button
                  onClick={() => { onMarkFound && onMarkFound(detail.id); setDetail(null); }}
                  className="h-10 rounded-md w-full bg-green-600 text-white text-sm font-medium flex items-center justify-center gap-2"
                >
                  <CheckCircle2 size={16} /> Mark Found
                </button>
              )}
