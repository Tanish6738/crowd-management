...
const Sidebar = ({ tabs, active, onChange, mobileOpen = false, onClose }) => {
  return (
    <>
      ...
      {/* Mobile slide-over */}
      <div className={`sm:hidden fixed inset-0 z-40 transition ${mobileOpen ? '' : 'pointer-events-none'}`} aria-hidden={!mobileOpen}>
        <div
          className={`absolute inset-0 bg-black/30 backdrop-blur-sm transition-opacity ${mobileOpen ? 'opacity-100' : 'opacity-0'}`}
          onClick={onClose}
        />
        <nav className={`absolute left-0 top-0 h-full w-64 max-w-[80%] bg-white border-r border-gray-200 shadow-xl transform transition-transform ${mobileOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col`} aria-label="Primary Mobile">
