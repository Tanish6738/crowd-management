...
# Mount CCTV video files directory
app.mount("/cctv", StaticFiles(directory="cctv"), name="cctv")
# Note: Removed streams mount as we're using simple streaming without HLS
...
# CCTV Streaming Endpoints
@app.post("/cctvs/{cctv_id}/start-stream")
def start_cctv_stream(cctv_id: str):
    """Start streaming for a CCTV device"""
    try:
        # Verify CCTV exists
        cctv = collection.find_one({"_id": ObjectId(cctv_id)})
        if not cctv:
            raise HTTPException(status_code=404, detail="CCTV not found")
        
        # Check if already streaming
        if cctv_id in active_streams:
            return {"message": "Stream already active", "status": "active"}
        
        # Get video source and type from CCTV data
        video_source = cctv.get('video_source', f"rtsp://{cctv.get('ip_address', 'localhost')}:{cctv.get('port', 554)}/stream")
        source_type = cctv.get('source_type', 'http')  # Default to http for RTSP streams
        
        # Start stream using the real streaming function
        success = start_simple_stream(cctv_id, video_source, source_type, active_streams)
...
@app.get("/cctvs/{cctv_id}/direct-video-url")
def get_direct_video_url(cctv_id: str):
    """Get direct video URL for a CCTV device"""
    try:
        # Verify CCTV exists
        cctv = collection.find_one({"_id": ObjectId(cctv_id)})
        if not cctv:
        ...
        video_source = cctv.get('video_source')
        source_type = cctv.get('source_type', 'file')
        
        if not video_source:
            raise HTTPException(status_code=404, detail="No video source configured")
        ...
        if source_type == 'file':
            if video_source.startswith('http'):
                ...
                # Local file - extract filename and create URL
                filename = video_source.split('/')[-1]
                video_url = f"/cctv/{filename}"
...
@app.get("/cctvs/active-streams")
def get_all_active_streams():
    """Get all active streams with their video URLs"""
    try:
        active_stream_data = []
        
        # Get all CCTVs that are marked as active
        active_cctvs = collection.find({"status": "active"})
        
        for cctv in active_cctvs:
            cctv_id = str(cctv["_id"])
            video_source = cctv.get('video_source')
            source_type = cctv.get('source_type', 'file')
        ...
        return {
            "active_streams": active_stream_data,
            "count": len(active_stream_data)
        }
    except Exception as e:
        logger.error(f"Error getting active streams: {e}")
        raise HTTPException(status_code=500, detail=f"Error getting active streams: {str(e)}")

@app.post("/cctvs/start-all-streams")
def start_all_cctv_streams():
    """Start streaming for all active CCTV devices"""
    try:
        # Get all active CCTVs
        active_cctvs = list(collection.find({"status": "active"}))
            ...
            try:
                ...
                if not video_source:
                    results.append({
                        ...
                        "name": cctv.get("name", "Unknown"),
                        "status": "error",
                        "message": "No video source configured"
                    })
                    continue
        ...
        return {
            "message": f"Processed {len(active_cctvs)} cameras",
            "success_count": success_count,
            "total_count": len(active_cctvs),
            "results": results
        }
        
    except Exception as e:
        logger.error(f"Error starting all streams: {e}")
        raise HTTPException(status_code=500, detail=f"Error starting all streams: {str(e)}")

@app.post("/cctvs/stop-all-streams")
def stop_all_cctv_streams():
    """Stop streaming for all CCTV devices"""
    try:
        active_stream_ids = list(active_streams.keys())
        results = []
        ...
        return {
            "message": f"Stopped {len(active_stream_ids)} streams",
            "results": results
        }
        
    except Exception as e:
        logger.error(f"Error stopping all streams: {e}")
        raise HTTPException(status_code=500, detail=f"Error stopping all streams: {str(e)}")

@app.post("/cctvs/{cctv_id}/stop-stream")
def stop_cctv_stream(cctv_id: str):
    """Stop streaming for a CCTV device"""
    try:
        # Verify CCTV exists
        cctv = collection.find_one({"_id": ObjectId(cctv_id)})
        if not cctv:
        ...
        if cctv_id not in active_streams:
            return {"message": "No active stream found", "status": "inactive"}
        
        # Stop stream using the placeholder function
        stop_simple_stream(cctv_id, active_streams)
...
@app.get("/cctvs/{cctv_id}/stream-status")
def get_cctv_stream_status(cctv_id: str):
    """Get streaming status for a CCTV device"""
    try:
        # Verify CCTV exists
        cctv = collection.find_one({"_id": ObjectId(cctv_id)})
        if not cctv:
        ...
        is_active = cctv_id in active_streams
        stream_info = active_streams.get(cctv_id, {})
        ...
        logger.error(f"Error getting CCTV stream status: {e}")
        raise HTTPException(status_code=500, detail=f"Error getting stream status: {str(e)}")

@app.get("/cctvs/{cctv_id}/current-frame")
def get_cctv_current_frame(cctv_id: str):
    """Get current frame from a CCTV device stream as file URL"""
    try:
        # Verify CCTV exists
        cctv = collection.find_one({"_id": ObjectId(cctv_id)})
        if not cctv:
        ...
        if cctv_id not in active_streams:
            return {
                "cctv_id": cctv_id,
                "status": "no_stream",
                "frame_url": "/static/placeholder-camera.jpg",
                "message": "Stream is not active. Start stream first.",
                "timestamp": datetime.now().isoformat()
            }
        
        # Get current frame using the real streaming function
        frame_base64 = get_current_frame(cctv_id, active_streams)
        
        if frame_base64 is None:
            return {
                "cctv_id": cctv_id,
                "status": "no_frame",
                "frame_url": "/static/no-signal.jpg",
                "message": "No frame available at the moment",
                "timestamp": datetime.now().isoformat()
            }
        ...
        # Save frame as file
        frame_filename = f"frame_{cctv_id}_{int(time.time() * 1000)}.jpg"
        ...
        return {
            ...
            "frame_url": frame_url,
            "timestamp": datetime.now().isoformat(),
            "message": "Frame captured and saved successfully"
        }
        
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid CCTV ID")
    except Exception as e:
        logger.error(f"Error getting CCTV current frame: {e}")
        raise HTTPException(status_code=500, detail=f"Error getting current frame: {str(e)}")

@app.get("/api/frames/{filename}")
def serve_frame_image(filename: str):
    """Serve generated frame images"""
    try:
        # For demo purposes, return a placeholder image or generate one
        # In real implementation, this would serve actual captured frames
        ...
        # If frame doesn't exist, create a simple placeholder
        if not frame_path.exists():
            # Return a default camera placeholder
            return FileResponse("static/camera-placeholder.jpg", media_type="image/jpeg")
        
        return FileResponse(str(frame_path), media_type="image/jpeg")
        
    except Exception as e:
        logger.error(f"Error serving frame image: {e}")
...
def create_placeholder_images():
    ...
    # Create simple text files that will be treated as placeholder images
    # In a real implementation, you would generate actual image files
    placeholder_files = [
    ...
    for filename in placeholder_files:
        file_path = static_dir / filename
        if not file_path.exists():
            # Create a simple text file as placeholder (normally would be an image)
...
# Video Processing Functions (real-time streaming with YOLO)
import base64
import io
